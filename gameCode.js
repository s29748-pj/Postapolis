...
const dialogues = [
  ...
  {
    id: "Nav5", characterId: "Nav", text: "Oh believe me, they are THAT bad. compared to them your... friend with a collar is a saint. believe me or not, but looking at what's left of them... i think they are the reason world ended",
    options: [
      { text: "C'mon, they can't be that bad. why end the world thak makes you money", nextDialogueId: "Nav6" }
    ]
  },
  {
    id: "Nav6", characterId: "Nav", text: "I've seen whole villages starving while so called CEO's grow so fat you could roll them. I've seen atrocities they are willing to commit... on humans. you see a city of slaves dear igu. And ksekses? you can't even imagine what they do to them. looking at it i feel that ksekse's wrath and hatred is justified...",
    options: [
      { text: "...", nextDialogueId: "Nav7" }
    ]
  },
  {
    id: "Nav7", characterId: "Nav", text: "...But i am sure they will take care of you. Sorry for... saying all of that.",
    options: [
  ...
  },
  {
    id: "Nav_outro",
    characterId: "Nav",
    text: "Navigator nods, stands next to you in silence while looking at the city. After a while, he goes back to the camp, leaving you with your own thoughts.",
    options: [
...
// Display a message for the current day
function displayDayMessage() {
  dialogueBox.textContent = dayMessages[days];
  situationElement.textContent= "A day like no other...";
}
...
// Increment day and update location
function incrementDay() {
  days++;
  console.log(`Day: ${days}`);
  changeLocation(((days - 1) % locations.length) + 1);
  displayDayMessage();
}
...
function startDialogue(dialogueId) {
  ...
  if (!dialogue) return console.error("Dialogue not found: ", dialogueId);
  ...
  if (character) {
    ...
    situationElement.textContent = `Talking to ${character.name}`;
  }
...
// Handle events for the current day
function handleDayEvents() {
  displayDayMessage();
  if (days === 1) startDialogue(1);
  if (days === 5) startDialogue("Nav1");
}

// Go to tent and prepare for sleep
function goToTent() {
  changeLocation(1);
  if (days === 1) {
    dialogueBox.textContent = "How are you supposed to sleep after this?";
    situationElement.textContent = "Still at Tent";
  } else {
    dialogueBox.textContent = "After a long day, Gehi assembles your tent at the heart of a camp. You are tired, everything hurts and your bed looks so comfy right now...";
    situationElement.textContent = "Back at Tent";
  }
  optionsContainer.innerHTML = "";
  avatarElement.src = "";
  optionsContainer.appendChild(createButton("Go to sleep", sleepAndProceed));
}
...
function fadeInScreenOverlayWithMessages() {
  ...
  function showNextMessage() {
    if (messageIndex < messages.length) {
      overlayMessage.textContent = messages[messageIndex];
      messageIndex++;
...
// End the game with a fade-out effect
function endGame() {
  overlay.style.opacity = "1";
  overlay.style.pointerEvents = "auto";
  overlayMessage.textContent = "The End";
  setTimeout(() => {
...
document.addEventListener("DOMContentLoaded", () => {
  fadeInScreenOverlayWithMessages();
  avatarElement.style.display = "none";
  changeLocation(1);
  handleDayEvents();
});
